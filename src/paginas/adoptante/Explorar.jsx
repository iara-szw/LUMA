...
export default function Explorar() {
  ...
  const perroKey="67fdca6b-e83d-4b2f-a374-b777a1e70037"
  const gatoKey="64c51957-d60e-4f24-be64-f6830572cb95"
  useEffect(() => {
    const obtenerDatos = async () => {
      const { data: perros } = await supabase
        .from('mascotas').select('id, nombre, tipo, edad, foto_url, urgente')
        .eq('especie', perroKey).eq('estado', 1)
      const { data: gatos } = await supabase
        .from('mascotas').select('id, nombre, tipo, edad, foto_url, urgente')
        .eq('especie', gatoKey).eq('estado', 1)
